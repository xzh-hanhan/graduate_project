...
class Emotion_Rec:
    ...
    def run(self, frame_in, canvas, label_face, label_result):
        ...
        frameClone =self.segmentation(frameClone,label)
        # 调整画面大小与界面相适应
        frameClone = cv2.resize(frameClone, (420, 520))
        #---
        #seg_image=cv2.resize(seg_image,(420,520))
    ...
    def segmentation(self,images,results):
        path=delete_background(images)
        img = mpimg.imread(path)
        if results == "angry":
            output_path=blend_images(path, "./images_test/angry.jpg")
            img = mpimg.imread(output_path)
            return img
        elif results == "surprised":
            output_path=blend_images(path, "./images_test/surprised.jpg")
            img = mpimg.imread(output_path)
            return img
        elif results == "sad":
            output_path=blend_images(path, "./images_test/sad.jpg")
            img = mpimg.imread(output_path)
            return img
        elif results == "happy":
            output_path=blend_images(path, "./images_test/happy.jpg")
            img = mpimg.imread(output_path)
            return img
        elif results == "disgust":
            output_path=blend_images(path, "./images_test/disgust.jpg")
            img = mpimg.imread(output_path)
            return img
        elif results == "scared":
            output_path=blend_images(path, "./images_test/scared.jpg")
            img = mpimg.imread(output_path)
            return img
        else:
            output_path = blend_images(path, "./images_test/neutral.jpg")
            img = mpimg.imread(output_path)
            return img
        return img;

    def show_image(self,label_face):
        # 在Qt界面中显示人脸
        frameClone = self.segmentation()
        show = cv2.cvtColor(frameClone, cv2.COLOR_BGR2RGB)
        showImage = QtGui.QImage(show.data, show.shape[1], show.shape[0], QtGui.QImage.Format_RGB888)
        label_face.setPixmap(QtGui.QPixmap.fromImage(showImage))
        QtWidgets.QApplication.processEvents()
